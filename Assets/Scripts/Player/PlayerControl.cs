...
public class PlayerControl : MonoBehaviour
{
    ...
    [Tooltip("Akselerasi saat mulai melangkah (m/s^2)")]
    public float acceleration = 35f;
    [Tooltip("Deselerasi/pengereman alami saat tombol dilepas (m/s^2)")]
    public float deceleration = 25f;
    [Tooltip("Kontrol arah saat berada di udara (m/s^2)")]
    ...
    void Update()
    {
        ...
        if (isPlanting || isAttacking)
        {
            ...
            if (animator != null)
            {
                animator.SetFloat("Vel", 0f, 0.12f, Time.deltaTime);
    ...
    void FixedUpdate()
    {
        // Kunci input: hentikan fisika pergerakan saat terkunci atau sedang menanam/menyerang secara mulus
        if (isInputLocked || isPlanting || isAttacking)
        {
            if (rb != null)
            {
                Vector3 curH = new Vector3(rb.linearVelocity.x, 0f, rb.linearVelocity.z);
                Vector3 stoppedH = Vector3.MoveTowards(curH, Vector3.zero, deceleration * 1.5f * Time.fixedDeltaTime);
                rb.linearVelocity = new Vector3(stoppedH.x, rb.linearVelocity.y, stoppedH.z);
            }
            return;
        }

        Vector3 currentHorizontalVel = rb != null ? new Vector3(rb.linearVelocity.x, 0f, rb.linearVelocity.z) : Vector3.zero;
